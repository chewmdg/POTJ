...
{
    public class Verb_LaunchProjectileJedi : Verse.Verb
    {
        ...
        public ProjectilePropertiesJedi projectilePropsCR
        {
            get
            {
                return this.projectileDef.projectile as ProjectilePropertiesJedi;
            }
        }
        ...
        public Pawn ShooterPawn
        {
            get
            {
                ...
                return Jedi_Utility.TryGetTurretOperator(this.caster);
            }
        }
        ...
        {
            const float gravity = Jedi_Utility.gravityConst;
            float angle = 0;
        ...
        protected virtual Vector3 ShiftTarget(ShiftVecReport report)
        {
            ...
            {
                targetableHeight = Jedi_Utility.GetCollisionHeight(this.currentTarget.Thing);
                if (report.cover != null)
                {
                    targetableHeight += Jedi_Utility.GetCollisionHeight(report.cover);
                }
                heightDifference += targetableHeight * 0.5f;    //Optimal hit level is halfway
            }

            this.shotHeight = Jedi_Utility.GetCollisionHeight(this.caster);
            if (this.CasterPawn != null)
            {
        ...
        public override bool CanHitTargetFrom(IntVec3 root, LocalTargetInfo targ)
        {
            ...
            if (base.CanHitTargetFrom(root, targ))
            {
                ...
                {
                    float targetHeight = Jedi_Utility.GetCollisionHeight(targ.Thing);
                    if (targetHeight <= Jedi_Utility.GetCollisionHeight(coverTarg))
                    {
                        return false;
                    }
                }
                ...
                {
                    float shotHeight = Jedi_Utility.GetCollisionHeight(this.caster);
                    if (this.CasterPawn != null)
                    {
                        shotHeight *= shotHeightFactor;
                    }
                    if (shotHeight <= Jedi_Utility.GetCollisionHeight(coverShoot))
                    {
                        return false;
                    }
                }
        ...
        protected override bool TryCastShot()
        {
            ...
            for (int i = 0; i < this.projectilePropsCR.pelletCount; i++)
            {
                ...
                ProjectileJedi projectile = (ProjectileJedi)ThingMaker.MakeThing(projectileDef, null);
                ...
                {
                    projectile.LaunchVector3(this.caster, casterExactPosition, new LocalTargetInfo(this.currentTarget.Thing), targetVec3, this.ownerEquipment);
                }
                else
                {
                    projectile.LaunchVector3(this.caster, casterExactPosition, new LocalTargetInfo(shootLine.Dest), targetVec3, this.ownerEquipment);
                }
            }
            this.numShotsFired++;
            return true;
